...
const productReducer = (state = defaultState, action) => {
    switch (action.type) {
        ...
        case "REMOVE_CARD":
            return {...state , card: state.card.filter((el,id) => el.id !== action.payload.id)}
        case "DECREASE_QUANTITY":
            if (state.card[action.payload].quantity > 1){
                return {
                    ...state, card: state.card.map((el , idx) => idx === action.payload ?
